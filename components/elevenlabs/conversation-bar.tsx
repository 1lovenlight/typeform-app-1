...
export interface ConversationBarProps {
  ...
  /**
   * Whether a character is selected (used for animating Play button)
   */
  hasSelectedCharacter?: boolean;

  /**
   * Callback when practice session is created (returns session_id)
   */
  onSessionCreated?: (sessionId: string) => void;

  /**
   * Callback when conversation ends (fires on disconnect with session_id)
   */
  onConversationEnd?: (sessionId: string | null) => void;

  /**
   * User ID for creating practice sessions
   */
  userId?: string;

  /**
   * Character ID for practice session tracking
   */
  characterId?: string;

  /**
   * Character name for practice session tracking
   */
  characterName?: string;

  /**
   * Activity ID for practice session tracking
   */
  activityId?: string;
}
...
>(
  (
    ...
    const { createSession, clearSession } = usePracticeSession();
    ...
    const conversation = useConversation({
      ...
      onDisconnect: () => {
        ...
        // Notify parent about conversation end with session ID
        if (currentSessionId) {
          onConversationEnd?.(currentSessionId);
        }
      },
      onMessage: (message) => {
        onMessage?.(message);
    ...
    const startConversation = React.useCallback(async () => {
      try {
        ...
        // Create practice session if userId is provided
        if (userId) {
    ...
    return (
      ...
      >
        <Card className="m-0 w-full gap-0 border-none p-0 shadow-lg bg-card-active rounded-xl">
            ...
            <div>
              ...
              <div className="flex items-center justify-between gap-2 p-2">
                <div className="flex flex-row items-center justify-between gap-2 w-full">
                  ...
                  {/* Connection banner - appears on top visually due to flex-col-reverse */}
                  {(agentState === "connected" ||
                    agentState === "connecting") && (
                    <div className="flex flex-row items-center justify-center gap-2 rounded-md px-4 w-fit h-9">
                      <div className="shrink-0 size-3 animate-pulse bg-destructive rounded-full"></div>
